...
#'
#' @param X1 data frame with data for treatment 1
#' @param X2 data frame with data for treatment 2
#' @param boot.out1 list containing output from boot.WAD.func or boot.TUBE.func for treatment 1 (corresponding to X1; the 'light' treatment if a 1-tailed permutation test is specified)
#' @param boot.out2 list containing output from boot.WAD.func or boot.TUBE.func for treatment 2 (corresponding to X2; the 'heavy' treatment if a 1-tailed permutation test is specified)
#' @param var variable name for the treatment ID columns in the data frames
#' @param CI confidence interval (0-1); default is 0.90
#' @param draws number of iterations for the permutation test
#' @param tailed.test specifies whether a 2-tailed or 1-tailed permutation test should be performed in calculating the p-value; default is 2; note: if 1-tailed is specified, then the function assumes group 2 (X2) is the 'heavy' treatment and group 1 (X1) is the light treatment
#'
#' @return A list with
#' \itemize{
...
#' }
#'
#' @note requires the functions 'WAD.func', 'boot.WAD.func', & 'comparison.message'
#' @note names of the x, y, replicate ID, and treatment ID columns in the two data frames must match exactly
#' @note number of reps need not be equal between the two groups
#' @note the test statistic for the P-value calculation is the difference in means of the 2 groups (calculated as group2 - group1)
#' @note the P-value estimates the probability that the observed difference (group2-group1) is as extreme or more extreme than would be expected if group identities were randomized
#'
#' @export


diff_wad_calc <- function(X1, X2, boot.out1, boot.out2, var="trt.code", CI=0.90, draws=1000, tailed.test=2){
  ...
  group1.names <- X1 %>% pull(var) %>% unique() %>% droplevels()
  group2.names <- X2 %>% pull(var) %>% unique() %>% droplevels()
  ...
  group.names <- c(paste(sort(group1.names), collapse = "_"), paste(sort(group2.names), collapse = "_"))
  ...
  message <- comparison_message(X.light=X1, X.heavy=X2, boot.out.light=boot.out1, boot.out.heavy=boot.out2, var=var)
...
#' Calculate WAD Differences (Deprecated)
#'
#' @keywords internal
#' @export

diff.wad.calc = function(...) {
  .Defunct("diff_wad_calc()")
}
