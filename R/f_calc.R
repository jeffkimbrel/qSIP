...
#' @param boot.out.light list containing output from boot.TUBE.func for the 'light' treatment (corresponding to X.light)
#' @param boot.out.heavy list containing output from boot.TUBE.func for the 'heavy' treatment (corresponding to X.heavy)
...
#' }
#'
#' @note requires the functions 'WAD.func', 'boot.TUBE.func', 'MW.calc', & 'comparison.message'
...
f_calc <- function(X.light, X.heavy, boot.out.light, boot.out.heavy, MW.out, iso.compare, days, var="trt.code", growth.model="exponential", prop.O.from.water=0.50, copies.cell=6, pgC.cell=0.1, CI=0.90){
