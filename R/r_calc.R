#' Calculate R
#'
#' Given two treatments and the incubation period, returns an estimate (with uncertainty) of the intrinsic rate of increase (r, units for exponential model: per day; units for linear model: copies per day per g soil) based on the degree of labeling by the heavy isotope
#'
...
#' @param boot.out.light list containing output from boot.WAD.func or boot.TUBE.func for the 'light' treatment (corresponding to X.light); note that if a linear growth model is used, a list containing output from boot.TUBE.func is required
#' @param boot.out.heavy list containing output from boot.WAD.func or boot.TUBE.func for the 'heavy' treatment (corresponding to X.heavy); note that if a linear growth model is used, a list containing output from boot.TUBE.func is required
...
#' @param days duration of the incubation period in days
...
#' @return A list with
#' \itemize{
#'   \item boot.r: vector of bootstrapped estimates of intrinsic rate of increase (per day OR copies per day per g soil) based on the two treatments
#'   \item obs.r: observed intrinsic rate of increase based on the two treatments
#'   \item boot.r.mean: mean of the bootstrapped intrinsic rate of increase values
#'   \item boot.r.median: median of the bootstrapped intrinsic rate of increase values
#'   \item boot.r.CI: upper and lower confidence intervals (defined by argument 'CI') of the bootstrapped intrinsic rate of increase values
...
#' }
#'
#' @note requires the functions 'WAD.func', 'boot.WAD.func', 'boot.TUBE.func', 'MW.calc', & 'comparison.message'
#' @note number of reps need not be equal between the two groups
#'
#' @details Assumptions:
#' * assumes either a linear growth model ( Nt=N0+(rt) ) or an exponential growth model ( Nt=N0*e^(rt) )
...
r_calc <- function(X.light, X.heavy, boot.out.light, boot.out.heavy, MW.out, iso.compare, days, var="trt.code", growth.model="exponential", prop.O.from.water=0.50, CI=0.90){
  ...
  if (is.na(iso.compare)){                                            #if the iso.compare code is 'NA', print an error message
    ...
    stop(paste("Error: unable to calculate r for the specified isotope comparison '", iso.compare, "'", sep=""))
  }
  if (growth.model=="linear" & (is.null(boot.out.light$obs.wads$tot.copies) | is.null(boot.out.heavy$obs.wads$tot.copies))){    #if growth.model is "linear" and the full output from 'boot.TUBE.func' is not present, print an error message
    stop("Error: unable to calculate r for a linear growth model without data on total number of copies\n  Use output from 'boot.TUBE.func' for the 'boot.out.light' & 'boot.out.heavy' arguments")
  }
  else if (growth.model=="linear" & !is.null(boot.out.light$obs.wads$tot.copies) & !is.null(boot.out.heavy$obs.wads$tot.copies)){   #if growth.model is "linear" (& the appropriate output from 'boot.TUBE.func' is present), calculate r using a linear growth model
