# This code imports the MH FW qSIP data sets and performs the basic calculations on the data
...
#Import raw data & taxomic information and format raw data for analysis:
  #Read in taxonomic information
    taxa.id <- read.table("qSIP_data/MH_FW_taxa_id.txt", header=T, sep="\t", stringsAsFactors=T, na.strings="")
  ...
  #NOTE: treatment code names cannot contain spaces
    data <- read.table("qSIP_data/MH_FW_qSIP_data.txt", header=T, sep="\t", stringsAsFactors=T)
    names(data)
  #Note that columns 9 and up have a unique identifier code for taxon to match that in the "taxa.id" data frame
    names(data)[9:dim(data)[2]]
  #Rename these 'taxonID' columns to just the numbers (i.e., get rid of the leading 'X')
    names(data)[9:dim(data)[2]] <- gsub(pattern="X(\\d+)", replacement="\\1", x=names(data)[9:dim(data)[2]], perl=TRUE)
    
  #Add a column for the sum of proportional abundance of all taxa by fraction
    data$sum.abundance <- rowSums(data[,9:ncol(data)])
    data$sum.abundance
  ...
  #Calculate number of copies per uL, based on relative abundance and total number of copies per uL:
    ncopies <- data$qPCR.16S.copies.ul*data[,9:(ncol(data)-1)]
    ncopies <- cbind(data[,1:8], ncopies)  # add first 8 columns of data to ncopies
  ...
  #Do this for copies.ul and for relative abundance. Merge these to into 1 masterfile: data.melted
    ncopies.melted <- melt(ncopies, id=c("tube", "sample", "tmt", "rep", "fraction", "density.g.ml", "DNA.ng.ul", "qPCR.16S.copies.ul"), measure.vars=as.character(1:236), variable.name="taxon", value.name="copies.ul")
    rel.abundance.melted <- melt(data, id=c("tube", "sample", "tmt", "rep", "fraction", "density.g.ml", "DNA.ng.ul", "qPCR.16S.copies.ul"), measure.vars=as.character(1:236), variable.name="taxon", value.name="rel.abundance")
...
#NOTE: THE DURATION OF THE INCUBATION 'days' IS MADE-UP TO ENABLE RUNNING 'all.taxa.calcs' BELOW:
  Tcompare <- read.table("qSIP_data/MH_FW_TreatmentComparisons.txt", header=TRUE, sep="\t", colClasses=c("numeric","factor","character","character","character","numeric","character"))
  summary(Tcompare)
  Tcompare
...
#Run all wad.diff, ape, r, & flux calculations for all taxa and all comparisons:
  ...
  set.seed(100)
  system.time(all.comparisons <- all.taxa.calcs(X.all=data.melted, comparisons=Tcompare, vars=c("taxon", "density.g.ml", "copies.ul", "tube", "tmt", "g.soil"), growth.model="exponential", prop.O.from.water=0.33, v.frac=50, copies.cell=6, pgC.cell=0.1, CI=0.90, draws=1000, tailed.test=1))
  ...
  new.bootstrapped.filenames <- paste("qSIP_output/", "MH_FW_", c("bootstrapped_wad1.txt", "bootstrapped_wad2.txt", "bootstrapped_wad_diff.txt", "bootstrapped_ape.txt"), sep="")
...
#Write the results (all.comparisons) to a text file:
  ...
  write.table(all.comparisons, "qSIP_output/MH_FW_all_comparisons.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)


#Write the taxa.id dataframe to a text file:
  write.table(taxa.id, "qSIP_output/MH_FW_taxa_ID.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
