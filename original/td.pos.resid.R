td.pos.resid <- function(DATA, lab.replicate=lab.replicate, min.num.nongrowers=min.num.nongrowers){
  ...
  for (j in 0:(dim(curr.data.lowSD)[1] - min.num.nongrowers)){
    if (j == 0){
      ...
      nxt.index.to.drop <- which(as.numeric(resid(mod1)) == max(as.numeric(resid(mod1))))
      indices.to.keep <- which(as.numeric(resid(mod1)) < max(as.numeric(resid(mod1))))
