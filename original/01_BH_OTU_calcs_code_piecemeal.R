...
#Create a function to subset the data.melted data frame into smaller chunks:
  dm.subset <- function(full.data, start, end){
    new.data <- full.data[full.data$taxon %in% levels(full.data$taxon)[start:end],]
      #Re-convert the factor columns to factor:
        new.data$taxon <- factor(new.data$taxon)
        new.data$SampleID <- factor(new.data$SampleID)
        new.data$trt.code <- factor(new.data$trt.code)
        new.data$tube <- factor(new.data$tube)
        new.data$kingdom <- factor(new.data$kingdom)
        new.data$phylum <- factor(new.data$phylum)
        new.data$class <- factor(new.data$class)
        new.data$order <- factor(new.data$order)
        new.data$family <- factor(new.data$family)
        new.data$genus <- factor(new.data$genus)
        new.data$species <- factor(new.data$species)
    new.data
  }


#Create a small test data set for the all.taxa.calcs function that only has the first 20 OTUs:
  data.melted.1.20 <- dm.subset(full.data=data.melted, start=1, end=20)
  #To run the all.taxa.calcs function on this small test data set, pass 'data.melted.1.20' to the all.taxa.calcs script


#Create multiple smaller datasets to be passed to all.taxa.calcs in series (the results will then be glued back together):
  length(levels(data.melted$taxon))   #total number of taxa (OTUs) in data.melted
  data.melted.1.300 <- dm.subset(full.data=data.melted, start=1, end=300)
  data.melted.301.600 <- dm.subset(full.data=data.melted, start=301, end=600)
  data.melted.601.900 <- dm.subset(full.data=data.melted, start=601, end=900)
  data.melted.901.1200 <- dm.subset(full.data=data.melted, start=901, end=1200)
  data.melted.1201.1500 <- dm.subset(full.data=data.melted, start=1201, end=1500)
  data.melted.1501.1800 <- dm.subset(full.data=data.melted, start=1501, end=1800)
  data.melted.1801.2100 <- dm.subset(full.data=data.melted, start=1801, end=2100)
...
#Run all wad.diff, ape, r, & flux calculations for all taxa and all comparisons:
  ...
  #Run the analyses on each subset of the data:
    ...
    #For OTUs 1-300:
      set.seed(100)
      system.time(all.comparisons.1.300 <- all.taxa.calcs(X.all=data.melted.1.300, comparisons=Tcompare, vars=c("taxon", "density.g.ml", "copies", "tube", "trt.code", "g.soil"), growth.model="exponential", v.frac=50, CI=0.90, draws=1000, tailed.test=1))
      ...
      new.bootstrapped.filenames.1.300 <- paste("qSIP_output/", "BH_OTU_", c("bootstrapped_wad1", "bootstrapped_wad2", "bootstrapped_wad_diff", "bootstrapped_ape"), "_1-300.txt", sep="")
      file.rename(from=bootstrapped.filenames, to=new.bootstrapped.filenames.1.300)
      write.table(all.comparisons.1.300, "qSIP_output/BH_OTU_all_comparisons_1-300.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)

    #For OTUs 301-600:
      set.seed(100)
      system.time(all.comparisons.301.600 <- all.taxa.calcs(X.all=data.melted.301.600, comparisons=Tcompare, vars=c("taxon", "density.g.ml", "copies", "tube", "trt.code", "g.soil"), growth.model="exponential", v.frac=50, CI=0.90, draws=1000, tailed.test=1))
      new.bootstrapped.filenames.301.600 <- paste("qSIP_output/", "BH_OTU_", c("bootstrapped_wad1", "bootstrapped_wad2", "bootstrapped_wad_diff", "bootstrapped_ape"), "_301-600.txt", sep="")
      file.rename(from=bootstrapped.filenames, to=new.bootstrapped.filenames.301.600)
      write.table(all.comparisons.301.600, "qSIP_output/BH_OTU_all_comparisons_301-600.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)

    #For OTUs 601-900:
      set.seed(100)
      system.time(all.comparisons.601.900 <- all.taxa.calcs(X.all=data.melted.601.900, comparisons=Tcompare, vars=c("taxon", "density.g.ml", "copies", "tube", "trt.code", "g.soil"), growth.model="exponential", v.frac=50, CI=0.90, draws=1000, tailed.test=1))
      new.bootstrapped.filenames.601.900 <- paste("qSIP_output/", "BH_OTU_", c("bootstrapped_wad1", "bootstrapped_wad2", "bootstrapped_wad_diff", "bootstrapped_ape"), "_601-900.txt", sep="")
      file.rename(from=bootstrapped.filenames, to=new.bootstrapped.filenames.601.900)
      write.table(all.comparisons.601.900, "qSIP_output/BH_OTU_all_comparisons_601-900.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)

    #For OTUs 901-1200:
      set.seed(100)
      system.time(all.comparisons.901.1200 <- all.taxa.calcs(X.all=data.melted.901.1200, comparisons=Tcompare, vars=c("taxon", "density.g.ml", "copies", "tube", "trt.code", "g.soil"), growth.model="exponential", v.frac=50, CI=0.90, draws=1000, tailed.test=1))
      new.bootstrapped.filenames.901.1200 <- paste("qSIP_output/", "BH_OTU_", c("bootstrapped_wad1", "bootstrapped_wad2", "bootstrapped_wad_diff", "bootstrapped_ape"), "_901-1200.txt", sep="")
      file.rename(from=bootstrapped.filenames, to=new.bootstrapped.filenames.901.1200)
      write.table(all.comparisons.901.1200, "qSIP_output/BH_OTU_all_comparisons_901-1200.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)

    #For OTUs 1201-1500:
      set.seed(100)
      system.time(all.comparisons.1201.1500 <- all.taxa.calcs(X.all=data.melted.1201.1500, comparisons=Tcompare, vars=c("taxon", "density.g.ml", "copies", "tube", "trt.code", "g.soil"), growth.model="exponential", v.frac=50, CI=0.90, draws=1000, tailed.test=1))
      new.bootstrapped.filenames.1201.1500 <- paste("qSIP_output/", "BH_OTU_", c("bootstrapped_wad1", "bootstrapped_wad2", "bootstrapped_wad_diff", "bootstrapped_ape"), "_1201-1500.txt", sep="")
      file.rename(from=bootstrapped.filenames, to=new.bootstrapped.filenames.1201.1500)
      write.table(all.comparisons.1201.1500, "qSIP_output/BH_OTU_all_comparisons_1201-1500.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)

    #For OTUs 1501-1800:
      set.seed(100)
      system.time(all.comparisons.1501.1800 <- all.taxa.calcs(X.all=data.melted.1501.1800, comparisons=Tcompare, vars=c("taxon", "density.g.ml", "copies", "tube", "trt.code", "g.soil"), growth.model="exponential", v.frac=50, CI=0.90, draws=1000, tailed.test=1))
      new.bootstrapped.filenames.1501.1800 <- paste("qSIP_output/", "BH_OTU_", c("bootstrapped_wad1", "bootstrapped_wad2", "bootstrapped_wad_diff", "bootstrapped_ape"), "_1501-1800.txt", sep="")
      file.rename(from=bootstrapped.filenames, to=new.bootstrapped.filenames.1501.1800)
      write.table(all.comparisons.1501.1800, "qSIP_output/BH_OTU_all_comparisons_1501-1800.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)

    #For OTUs 1801-2100:
      set.seed(100)
      system.time(all.comparisons.1801.2100 <- all.taxa.calcs(X.all=data.melted.1801.2100, comparisons=Tcompare, vars=c("taxon", "density.g.ml", "copies", "tube", "trt.code", "g.soil"), growth.model="exponential", v.frac=50, CI=0.90, draws=1000, tailed.test=1))
      new.bootstrapped.filenames.1801.2100 <- paste("qSIP_output/", "BH_OTU_", c("bootstrapped_wad1", "bootstrapped_wad2", "bootstrapped_wad_diff", "bootstrapped_ape"), "_1801-2100.txt", sep="")
      file.rename(from=bootstrapped.filenames, to=new.bootstrapped.filenames.1801.2100)
      write.table(all.comparisons.1801.2100, "qSIP_output/BH_OTU_all_comparisons_1801-2100.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)


#Glue the results back together:
  all.comparisons <- rbind(all.comparisons.1.300, all.comparisons.301.600, all.comparisons.601.900, all.comparisons.901.1200, all.comparisons.1201.1500, all.comparisons.1501.1800, all.comparisons.1801.2100)
  summary(all.comparisons)
  dim(all.comparisons)
  
  write.table(all.comparisons, "qSIP_output/BH_OTU_all_comparisons.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)

  #Glue the bootstrapped ouptut together:
  wad1.list <- c(new.bootstrapped.filenames.1.300[1], new.bootstrapped.filenames.301.600[1], new.bootstrapped.filenames.601.900[1], new.bootstrapped.filenames.901.1200[1], new.bootstrapped.filenames.1201.1500[1], new.bootstrapped.filenames.1501.1800[1], new.bootstrapped.filenames.1801.2100[1])
  wad2.list <- c(new.bootstrapped.filenames.1.300[2], new.bootstrapped.filenames.301.600[2], new.bootstrapped.filenames.601.900[2], new.bootstrapped.filenames.901.1200[2], new.bootstrapped.filenames.1201.1500[2], new.bootstrapped.filenames.1501.1800[2], new.bootstrapped.filenames.1801.2100[2])
  wad_diff.list <- c(new.bootstrapped.filenames.1.300[3], new.bootstrapped.filenames.301.600[3], new.bootstrapped.filenames.601.900[3], new.bootstrapped.filenames.901.1200[3], new.bootstrapped.filenames.1201.1500[3], new.bootstrapped.filenames.1501.1800[3], new.bootstrapped.filenames.1801.2100[3])
  ape.list <- c(new.bootstrapped.filenames.1.300[4], new.bootstrapped.filenames.301.600[4], new.bootstrapped.filenames.601.900[4], new.bootstrapped.filenames.901.1200[4], new.bootstrapped.filenames.1201.1500[4], new.bootstrapped.filenames.1501.1800[4], new.bootstrapped.filenames.1801.2100[4])
  
  for (i in 1:length(wad1.list)){
    if (!exists("wad1.boots")){
      wad1.boots <- read.table(wad1.list[i], header=TRUE, sep="")
    }
    else if (exists("wad1.boots")){
      wad1.temp <- read.table(wad1.list[i], header=TRUE, sep="")
      wad1.boots <- rbind(wad1.boots, wad1.temp)
    }
  }
  write.table(wad1.boots, "qSIP_output/BH_OTU_bootstrapped_wad1.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
  
  for (i in 1:length(wad2.list)){
    if (!exists("wad2.boots")){
      wad2.boots <- read.table(wad2.list[i], header=TRUE, sep="")
    }
    else if (exists("wad2.boots")){
      wad2.temp <- read.table(wad2.list[i], header=TRUE, sep="")
      wad2.boots <- rbind(wad2.boots, wad2.temp)
    }
  }
  write.table(wad2.boots, "qSIP_output/BH_OTU_bootstrapped_wad2.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)

  for (i in 1:length(wad_diff.list)){
    if (!exists("wad_diff.boots")){
      wad_diff.boots <- read.table(wad_diff.list[i], header=TRUE, sep="")
    }
    else if (exists("wad_diff.boots")){
      wad_diff.temp <- read.table(wad_diff.list[i], header=TRUE, sep="")
      wad_diff.boots <- rbind(wad_diff.boots, wad_diff.temp)
    }
  }
  write.table(wad_diff.boots, "qSIP_output/BH_OTU_bootstrapped_wad_diff.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)

  for (i in 1:length(ape.list)){
    if (!exists("ape.boots")){
      ape.boots <- read.table(ape.list[i], header=TRUE, sep="")
    }
    else if (exists("ape.boots")){
      ape.temp <- read.table(ape.list[i], header=TRUE, sep="")
      ape.boots <- rbind(ape.boots, ape.temp)
    }
  }
  write.table(ape.boots, "qSIP_output/BH_OTU_bootstrapped_ape.txt", append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
