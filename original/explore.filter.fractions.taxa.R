#Define a function to use to filter the data so that only taxa with an appropriate level of occurrence among all fractions within replicate(s) of a treatment are retained for further analysis:
  ...
  explore.filter.fractions.taxa <- function(DATA, trt.code=NULL, vars=c("taxon", "density.g.ml", "copies", "tube", "trt.code"), min.fracs, min.reps){
    ...
    #Taxa filtered:
      ...
      print("Taxa occurring in the specified treatment: ", sep="")
      print(levels(factor(DATA.treatment[,vars[1]])))
      print(paste("Taxa occurring in ≥", min.fracs, " fractions of ≥ ", min.reps, " replicates of the specified treatment: ", sep=""))
      print(as.character(sort(as.numeric(taxa.to.keep))))
    ...
    #Now, explore what subsetting the DATA dataframe would mean so that it only contains taxon-tubes for taxa occurring in at least 'min.fracs' fractions of 'min.reps' tubes across the specified treatment:
